  ...
  describe 'callbacks' do
    describe '#include_areas_of_product' do
      it 'includes areas of product before validation' do     
        ...
        subject.product_id = Product.where(name: 'New Product').first.try(:id) || FactoryGirl.create(:product, name: 'New Product', area_ids: [FactoryGirl.create(:area, name: 'Area2').id]).id
